...
export const createPatient = (p: Patient) => {
	return `INSERT INTO patient (first_name, middle_name, 
...
VALUES ('${p.first_name}', '${p.middle_name}', '${p.last_name}', '${p.date_of_birth}', '${p.gender}',
    '${p.ssn}', '${p.insurance_number}',
     ...
     '${p.phone_number}', '${p.email_address}', '${p.user_id}', ${p.responsible_party_id})
RETURNING patient_id`;
};

export const getPatientInfo = (id: number) => {
	return `SELECT CONCAT(first_name,' ', last_name ) from patient as p
